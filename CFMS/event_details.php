...
if($_GET['id']){
    ...
    $sql="SELECT * FROM tbl_events WHERE event_id='$eventid' AND status=1";
    $result=mysqli_query($conn,$sql);
    $data=mysqli_fetch_array($result); 
    $eventname=$data['event_name'];
    $club_id=$data['club_id'];
    $sql2="SELECT * FROM tbl_clubs WHERE club_id = '$club_id' AND status=1";
...
if(isset($_POST['register'])){
    ...
    $regdate= date('Y-m-d');
    $sql3="INSERT into tbl_participants(participant_id,name,user_id,event_id,reg_date) values('$participant_id','$name','$user_id','$eventid','$regdate')";
...
if(isset($_POST['volunteer'])){
    ...
    $vol_id="VOL-".$id;
    $regdate= date('y-m-d');
    $sql4="INSERT into tbl_volunteer(vol_id,vol_name,user_id,event_id,reg_date) values('$vol_id','$name','$user_id','$eventid','$regdate')";
    $result4=mysqli_query($conn,$sql4);
    
    if($result4){
        echo '<div class="alert alert-success">Registration successful!</div>';
        include "volunteeremail.php";
    } else {
        echo '<div class="alert alert-danger">Registration failed.</div>';
    }
...
<head>
        <meta charset="utf-8" />
        <title>EVENT DETAILS | USER</title>
...
<div class="row">
   ...
   <form method="post" >
            ...
            $sql = "SELECT * FROM tbl_participants WHERE user_id='$user_id' AND event_id='$event_id' AND status = 1";
            $result = mysqli_query($conn, $sql);
            if (mysqli_num_rows($result) > 0) {
                echo '<button type="submit" name="register" class="btn btn-primary mt-3 mb-3 w-40" disabled>Participate Now</button>';
            } else {
                echo '<button type="submit" name="register" class="btn btn-primary mt-3 mb-3 w-40">Participate Now</button>';
            }
            ?>
            <?php
            $sqlVolunteerCheck = "SELECT * FROM tbl_volunteer WHERE user_id='$user_id' AND event_id='$event_id' AND status = 1";
            $resultVolunteerCheck = mysqli_query($conn, $sqlVolunteerCheck);
            if (mysqli_num_rows($resultVolunteerCheck) > 0) {
                echo '<button type="submit" name="volunteer" class="btn btn-primary mt-3 mb-3 w-40" disabled>Register As Volunteer</button>';
            } else {
                echo '<button type="submit" name="volunteer" class="btn btn-primary mt-3 mb-3 w-40">Register As Volunteer</button>';}?>
        </form>
    <center>
